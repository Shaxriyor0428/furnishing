...
export class ProductDetailController {
  ...
  @ApiResponse({ status: 201, description: 'Product detail created successfully.' })
  @ApiResponse({ status: 400, description: 'Bad Request' })
  async createProductDetail(@Body() createProductDetailDto: CreateProductDetailDto) {
    return this.productDetailService.create(createProductDetailDto);
  }

    @Get()
    @ApiOperation({ summary: 'Retrieve all product details with optional filtering, sorting, and pagination' })
    @ApiQuery({ name: 'filter', required: false, description: 'Filter by product detail name or description' })
  ...
  @ApiResponse({ status: 200, description: 'Product detail updated successfully.' })
  @ApiResponse({ status: 404, description: 'Product detail not found.' })
  async update(
    @Param('id') id: number,
  ...
  @ApiResponse({ status: 200, description: 'Product detail deleted successfully.' })
  @ApiResponse({ status: 404, description: 'Product detail not found.' })
